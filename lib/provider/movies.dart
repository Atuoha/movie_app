...
class Movies extends ChangeNotifier {
  bool isItemOnFav(String id) {
    return favoriteMovies.any((movie) => movie.id == id);
  }

  void toggleMovieToFav(String id) {
    var movie = availableMovies.firstWhere((movie) => movie.id == id);
    if (isItemOnFav(id)) {
  ...
  Movie_Data findById(String id) {
    return availableMovies.firstWhere((movie) => movie.id == id);
  }
